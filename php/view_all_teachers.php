...
$response = array('success' => false, 'teachers' => array(), 'total' => 0, 'message' => '');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $allowed_sorts = array('first_name', 'last_name', 'email', 'department', 'designation', 'created_at');
    ...
    // Get total count
    $countQuery = "SELECT COUNT(*) as total FROM teachers WHERE is_active = 1";
    $countResult = executeQuery($countQuery);
    $countRow = $countResult->fetch_assoc();
    $total = $countRow['total'];

    // Get teachers with pagination
    $query = "SELECT id, first_name, middle_name, last_name, email, phone, city, state,
                     department, designation, qualification, experience, created_at 
              FROM teachers 
              WHERE is_active = 1 
